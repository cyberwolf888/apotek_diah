            ...
            <div class="portlet light portlet-fit portlet-datatable bordered">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject font-green sbold uppercase">Kelola Item</span>
                    </div>
                    <div class="actions">
                        <div class="btn-group btn-group-devided">
                            <a href="{{ route('backend.item.create') }}" class="btn btn-circle green">
                    ...
                    <div class="table-container">
                        <table class="table table-striped table-bordered table-hover" id="sample_3">
                            ...
                            @foreach($model as $row)
                                <tr>
                                    <td> {{ $no }}</td>
                                    <td> <img src="{{ url('assets/img/items/'.$row->gambar) }}" style="width: 150px; height: 100px;"></td>
                                    <td> {{ $row->nama }}</td>
                                    <td> {{ $row->stock }}</td>
                                    <td> {{ number_format($row->harga,0,',','.') }}</td>
                                    <td> {{ $row->jenis == 1 ? 'Obat':'Peralatan Medis' }}</td>
                                    <td class="center" width="100">
                                        <a href="{{ route('backend.item.edit',$row->id) }}" class="btn yellow-saffron btn-xs"><i class="fa fa-pencil"></i></a>
                                    </td>
                                </tr>
                                <?php $no++ ?>
                            @endforeach
